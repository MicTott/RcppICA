...
RcppICA v0.2.0 introduces **sparse matrix support**, enabling memory-efficient ICA on extremely large single-cell datasets (1M+ cells). This vignette demonstrates how sparse matrices dramatically reduce memory usage and improve performance for large-scale gene module identification.
...
- **Scalability bottleneck**: 1M cells × 20K genes = 160GB dense, 16GB sparse

**RcppICA's sparse implementation**:
- Computes covariance without materializing dense centered matrix
- 5-10x memory savings
- 2-4x speedup
- Enables direct ICA on expression matrices that would otherwise cause OOM
...
time_sparse <- system.time({
  result_sparse <- fastICA(X_dense, n.comp = 10, verbose = FALSE, seed = 123)
})
...
test_sizes <- data.frame(
  n_cells = c(5000, 10000, 25000, 50000),
  n_genes = c(1000, 2000, 5000, 10000),
  sparsity = c(0.10, 0.10, 0.08, 0.05)
)
...
for (i in 1:nrow(test_sizes)) {
  ...
  # Time sparse ICA
  t <- system.time({
    res <- fastICA(X_sp, n.comp = 10, verbose = FALSE, seed = 42)
  })

  results <- rbind(results, data.frame(
    Dataset = sprintf("%dK×%d", n/1000, m),
...
✓ **Direct ICA on 1M+ cell datasets** on standard workstations
✓ **5-10x memory savings** for typical scRNA-seq sparsity
✓ **2-4x speed improvement** by avoiding densification
✓ **Automatic optimization** - just pass sparse matrix, we handle the rest

### Key Takeaways

1. **Always use sparse matrices** for scRNA-seq data when possible
2. **Sparse implementation is production-ready** - same accuracy, better performance
3. **Gene module identification** now practical on atlas-scale datasets
4. **No code changes needed** - automatic sparse/dense routing
