...
### Spectra-Based Whitening (Default)

**How it works**: Truncated eigendecomposition via Lanczos iteration (Spectra library). Computes only the top-k eigenvalues needed, rather than full decomposition.

**Advantages**:
- Fastest for typical use (extracting k << m components)
- Memory-efficient: only computes what's needed
- Scales well to high-dimensional data

```{r spectra-whitening}
# Spectra whitening (default)
result_spectra <- fastICA(X, n.comp = 3, whiten.method = "spectra", maxit = 200)
cat("Spectra whitening - Converged:", result_spectra@misc$converged, "\n")
```

### Eigendecomposition Whitening

**How it works**: Full eigendecomposition of the covariance matrix.

**Advantages**:
- Simple, well-understood approach
- Can be appropriate when extracting most components

**Disadvantages**:
- Computes all eigenvalues even if only k are needed
...
cors <- abs(cor(t(result_spectra@S), t(result_eigen@S)))
cat("\nCorrelation between Spectra and Eigen results:\n")
print(round(apply(cors, 1, max), 3))
```

### SVD-Based Whitening

**How it works**: Uses Singular Value Decomposition (BDCSVD from Eigen). For large matrices, falls back to the covariance-based approach.

**Advantages**:
- Most numerically stable
- Handles ill-conditioned data well

**Disadvantages**:
- Slower than Spectra for k << m
- Full decomposition for small matrices

### Whitening Selection Guide

**Use Spectra** (default) when:
- Standard use cases
- Extracting k << m components
- Speed and memory efficiency matter

**Use Eigen** when:
- Extracting most components (k close to m)
...
START
  ...
  |
  ├─ Whitening approach?
  |    ├─ Few components (k << m) → whiten.method = "spectra" (default)
  |    ├─ Ill-conditioned data → whiten.method = "svd"
  |    └─ Full decomposition needed → whiten.method = "eigen"
  |
  ├─ Speed vs Accuracy?
