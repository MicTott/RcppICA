...
RcppICA provides a fast, modern implementation of the FastICA algorithm using:

- **Spectra library** for efficient truncated eigendecomposition (computes only top-k eigenvalues)
- **Eigen C++ library** for optimized linear algebra
- **OpenMP parallelization** for multi-core processing
- **Optimized memory management** with pre-allocated matrices

**Performance**: Competitive with base fastICA, with 2-3× faster whitening compared to full eigendecomposition. Best performance on medium-to-large datasets (1000+ variables).
...
# Extract components (independent sources)
S <- result@S  # n.comp x observations
head(S[, 1:10], 3)  # First 10 observations

# Extract mixing matrix
A <- result@A  # variables x n.comp
print(A)
```
