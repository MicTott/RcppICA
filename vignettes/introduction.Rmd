...
RcppICA provides a fast implementation of the FastICA algorithm using:

- **Eigen C++ library** for optimized linear algebra with BLAS delegation
- **Spectra library** for truncated eigendecomposition (only computes top-k eigenvalues)
- **OpenMP parallelization** for multi-core processing
- **Memory-efficient whitening** that avoids materializing intermediate matrices
...
S <- result@S
cat("Components dimensions:", dim(S), "\n")

# Access mixing matrix
A <- result@A
cat("Mixing matrix dimensions:", dim(A), "\n")
```
...
X_large <- matrix(rnorm(100000), 10000, 100)
...
RcppICA is typically 2-7x faster than the fastICA package, with the advantage increasing at larger matrix sizes.
